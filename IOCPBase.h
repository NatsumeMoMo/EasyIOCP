...
#define BUFF_SIZE 8192000						// I/O 请求的缓冲区大小
...
#define EXIT_CODE	NULL						// 传递给Worker线程的退出信号,从(-1)改为NULL,对x64平台编译警告优化
#define DEFAULT_IP	("127.0.0.1")				// 默认IP地址
#define DEFAULT_PORT	(9527)					// 默认端口
...
private:
	...
	// 新增发送队列和相关成员
	std::queue<std::vector<char>> sendQueue;        // 发送数据队列
	std::mutex sendQueueMutex;                      // 发送队列的互斥锁
	bool isSending;
